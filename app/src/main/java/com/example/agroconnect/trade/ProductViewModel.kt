...
import com.example.agroconnect.ui.Utils
...
//                Log.d("ProductViewModel", "Response: $productList")
    ...
    fun searchProductsFashion(userId: Int) {
        viewModelScope.launch {
            try {

                val response = apiService.getAllFashion()
                Log.d(TAG, "Filtered Product List: ${response.body()}")
                val productResponse = response.body()
                val productList = productResponse?.fashionItems

                // Filter the product list based on userId
                val filteredProductList = productList?.filter { it.user.id == userId }
